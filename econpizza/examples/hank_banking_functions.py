...
def egm_step(VPrime, VaPrime, a_grid, skills_grid, w, n, T, R, Rb, beta, sigma_c, sigma_l):
    """A single backward step via EGM
    """

    # interest rates as a vector on a_grid
    Ract_nextgrid = jnp.where(a_grid > 0., R, Rb)
    ...
    Ract = jnp.where(a > 0., R, Rb)

    # calculate new Values & MUX 
    x = c - labor_inc/(1 + sigma_l)
    x = jnp.where(x < 1e-16, 1e-16, x) # helps during convergence, but check!
