...
def egm_step(Wa_p, a_grid, skills_grid, w, n, T, xi, R, beta, sigma_c):
    ...
    benefit_schedule_temp = jnp.concatenate([xi_reshaped, zeros_vector], axis=1)
...
def get_sam_grid_from_stationary(sigma, stationary_distribution):
    ...
    scaled_grid = jnp.exp(s) / jnp.sum(stationary_distribution[1:] * jnp.exp(s))
    
    augmented_grid = jnp.concatenate([jnp.array([0.0]), scaled_grid]) # unemployment state
...
def sam_markov(rho, sigma, N, find_rate, separate_rate):
    ...
    scaled_Pi = (1 - separate_rate) * Pi  # Scale the original transition matrix
